"use client";
import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";

const WHATSAPP_NUMBER = "+919810292527";

export default function WhatsAppButton() {
  const [showBubble, setShowBubble] = useState(false);
  const [dismissed, setDismissed] = useState(false);

  // Auto-show the bubble after 3 seconds, only once
  useEffect(() => {
    if (dismissed) return;
    const timer = setTimeout(() => setShowBubble(true), 3000);
    return () => clearTimeout(timer);
  }, [dismissed]);

  const openWhatsApp = () => {
    const message = encodeURIComponent(
      "Jai Gau Mata 🙏\nI would like to know more about Triveni Gau Sewa Trust and how I can help."
    );
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${message}`, "_blank");
  };

  const handleDismiss = (e: React.MouseEvent) => {
    e.stopPropagation();
    setShowBubble(false);
    setDismissed(true);
  };

  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[999] flex flex-col items-end gap-3">
      {/* Chat bubble popup */}
      <AnimatePresence>
        {showBubble && (
          <motion.div
            initial={{ opacity: 0, scale: 0.8, y: 10 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.8, y: 10 }}
            transition={{ type: "spring", stiffness: 300, damping: 22 }}
            className="relative max-w-[200px] sm:max-w-[240px] cursor-pointer"
            onClick={openWhatsApp}
          >
            {/* Bubble card */}
            <div
              className="rounded-2xl rounded-br-sm px-4 py-3 shadow-2xl border border-green-700/40"
              style={{
                background: "linear-gradient(135deg, #1a2e1a 0%, #0f1f0f 100%)",
                boxShadow: "0 8px 32px rgba(34,197,94,0.15), 0 2px 8px rgba(0,0,0,0.4)",
              }}
            >
              {/* Dismiss ✕ */}
              <div className="flex justify-end mb-1">
                <button
                  onClick={handleDismiss}
                  className="text-green-600/60 hover:text-green-400 transition-colors text-sm leading-none"
                  aria-label="Close"
                >
                  ✕
                </button>
              </div>

              {/* Message */}
              <p
                className="text-xs leading-relaxed"
                style={{
                  fontFamily: "'Lora', serif",
                  color: "rgba(187,247,208,0.85)",
                }}
              >
                🙏 Jai Gau Mata! How can we assist you today?
              </p>

              {/* Tap to chat hint */}
              <p
                className="text-green-500/50 text-[10px] mt-2 text-right tracking-wide"
                style={{ fontFamily: "'Lora', serif" }}
              >
                Tap to chat →
              </p>
            </div>

            {/* Tail triangle */}
            <div
              className="absolute -bottom-[7px] right-4 w-0 h-0"
              style={{
                borderLeft: "7px solid transparent",
                borderRight: "7px solid transparent",
                borderTop: "8px solid #0f1f0f",
              }}
            />
          </motion.div>
        )}
      </AnimatePresence>

      {/* WhatsApp button */}
      <motion.button
        onClick={() => {
          if (showBubble) {
            openWhatsApp();
          } else {
            setShowBubble((v) => !v);
            setDismissed(false);
          }
        }}
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.93 }}
        aria-label="Chat on WhatsApp"
        className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300"
        style={{
          background: "linear-gradient(135deg, #25d366 0%, #128c4e 100%)",
          boxShadow: "0 4px 24px rgba(37,211,102,0.45), 0 2px 8px rgba(0,0,0,0.3)",
        }}
      >
        {/* Pulse ring */}
        <span
          className="absolute inset-0 rounded-full animate-ping opacity-25"
          style={{ background: "#25d366" }}
        />

        {/* WhatsApp SVG icon */}
        <svg className="w-7 h-7 relative z-10" fill="white" viewBox="0 0 24 24">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
      </motion.button>
    </div>
  );
}
